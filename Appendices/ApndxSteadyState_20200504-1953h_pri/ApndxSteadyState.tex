\input{./econtexRoot}\documentclass[../BufferStockTheory.tex]{subfiles}\input{./econtexRoot}\usepackage{ApndxSteadyState}
\input{\econtexRoot/\LaTeXFiles/onlyinsubfile}\begin{document}
...
\end{comment}
  ...
  \end{align}\mrgn{Every $\point$ in $\SetSub_{k}$ has same pty $\pContAprx_{k}$}where $\mathbb{I}_{\SetSub}(\bullet)$ is the indicator function of a set $\SetSub \subseteq \DistSetSprtFull$ (that is, $\mathbb{I}_{\SetSub}(\point) = 1$ if $\point \in \SetSub$; else $\mathbb{I}_{\SetSub}(\point) = 0$).  % The approximation error is upper-bounded by  ... [boring details]
  ...
  \end{align}{\mrgn{Reversed the order of integration here; $\that$ first, then $\this$}}

...
(degenerately $\underline{\tShkEmp}=\bar{\tShkEmp}=1$).\footnote{See \cite{rabaultBorrowing} and \cite{lsIncFluct} for analyses of cases where the shock processes have unbounded support.}  Call the cumulative
...
The familiar analytical solution to the perfect foresight specialization of the model, obtained by setting $\pZero=0$ and $\underline{\tShkEmp}=\bar{\tShkEmp}=\underline{\pShk}=\bar{\pShk}=1$, allows us to define some remaining notation and terminology.

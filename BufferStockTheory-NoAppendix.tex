...
\ShorterYN{}{
  ...
  \end{align*}
  where the second-to-last line follows from \cite{szeidlInvariant}'s proof, which implies that $\Mean[a_{t+1}-a_{t}]=0$.
...
The only adjustments this requires to the analysis from prior parts of this paper are therefore to the few elements that involve a role for $\Rfree$ distinct from its contribution to $\Pat$ (including the {\RIC}).  %For example, the {\RIC}~($\Pat/(\Rfree /\Alive) < 1$) will be somewhat easier to satisfy because $\Rfree/\Alive > \Rfree$.
